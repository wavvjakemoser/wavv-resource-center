...
// ─── Constants ────────────────────────────────────────────────────────────────
const ACCENT = "#00A9E2";
const BANNER_URL = "/manus-storage/final-webinar-spotlight-transparent_00828638.png";
...
function WebinarRow({
  ...
  function handleWatchClick() {
    ...
    trackAnon.mutate({
      ...
      metadata: JSON.stringify({ id: webinar.id, title: webinar.title, type: "exclusive" }),
    });
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function WebinarLiveExclusive() {
  const { data: webinars, isLoading } = trpc.webinars.list.useQuery({ type: "exclusive" });
  ...
  return (
    <PortalLayout title="Live Exclusive Webinars">
      <div className="px-4 lg:px-8 py-6 space-y-6">
        ...
        >
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ opacity: 0.12 }} xmlns="http://www.w3.org/2000/svg">
            <defs>
              <pattern id="circuit-live" x="0" y="0" width="60" height="60" patternUnits="userSpaceOnUse">
            ...
            <rect width="100%" height="100%" fill="url(#circuit-live)"/>
          </svg>
          <div
          ...
          <div className="relative flex flex-col justify-center h-full px-8 py-6 gap-1">
            ...
            <h1 className="text-2xl font-extrabold text-white leading-tight">WAVV Live Exclusive Webinars</h1>
            <p className="text-sm text-gray-300 mb-2">Single-topic, focused live sessions with the WAVV team — register to attend.</p>
          ...
          <div className="relative w-full max-w-4xl rounded-2xl overflow-hidden" style={{ background: "#111", border: "1px solid #2a2a2a", boxShadow: "0 25px 80px rgba(0,0,0,0.7)" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="relative w-full" style={{ paddingBottom: "56.25%" }}>
              {playingVideo.isHosted ? (
                ...
                <iframe ref={modalIframeRef} src={buildEmbedUrlWithTime(addYouTubeApiParam(playingVideo.embedUrl), playingVideo.startTime || 0)} title={playingVideo.title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; fullscreen" allowFullScreen sandbox="allow-scripts allow-same-origin allow-presentation allow-forms" className="absolute inset-0 w-full h-full" style={{ border: "none" }} />
              )}
            </div>
